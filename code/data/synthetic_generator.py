...
class SyntheticTransactionGenerator:
    ...
    def __init__(
        self, n_transactions: int = 100000, fraud_rate: float = 0.023, seed: int = 42
    ):
        """
    ...
    def generate(self) -> pd.DataFrame:
        ...
        all_txns = all_txns.sample(frac=1, random_state=self.seed).reset_index(
            drop=True
        )
    ...
    def _generate_benign(self, n: int) -> pd.DataFrame:
        ...
        data = {
            ...
            "sender_country": self.rng.choice(
                ["US", "GB", "CA", "AU", "DE", "FR", "JP"], size=n
            ),
            "receiver_country": self.rng.choice(
                ["US", "GB", "CA", "AU", "DE", "FR", "JP"], size=n
            ),
            "transaction_type": self.rng.choice(
                ["transfer", "payment", "withdrawal"], size=n
            ),
            "is_fraud": 0,
            "fraud_typology": "none",
        }
        ...
        data["timestamp"] = [
            base_time + timedelta(seconds=self.rng.randint(0, 90 * 24 * 60 * 60))
            for _ in range(n)
        ]
    ...
    def _generate_fraud(self, n: int) -> pd.DataFrame:
        ...
        if remaining > 0:
            fraud_dfs.append(
                self._generate_typology(self.rng.choice(typology_names), remaining)
            )
    ...
    def _generate_structuring(self, n: int) -> pd.DataFrame:
        ...
        for cluster_size in cluster_sizes[:n]:
            ...
            cluster_time = base_time + timedelta(
                seconds=self.rng.randint(0, 90 * 24 * 60 * 60)
            )
    ...
    def _generate_rapid_movement(self, n: int) -> pd.DataFrame:
        ...
        for _ in range(n_chains):
            ...
            accounts = [
                f"USER_{self.rng.randint(60000, 70000):06d}"
            ...
            data.append(
                {
                    ...
                    "transaction_type": "transfer",
                    "timestamp": base_time
                    + timedelta(seconds=self.rng.randint(0, 90 * 24 * 60 * 60)),
                    "is_fraud": 1,
    ...
    def _generate_smurfing(self, n: int) -> pd.DataFrame:
        ...
        for _ in range(n_target_accounts):
            ...
            campaign_time = base_time + timedelta(
                seconds=self.rng.randint(0, 80 * 24 * 60 * 60)
            )
