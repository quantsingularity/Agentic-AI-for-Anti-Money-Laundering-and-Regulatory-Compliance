...
class PrivacyGuard(BaseAgent):
        ...
        if isinstance(input_data, dict):
            ...
            redacted_data = [
                (
                    ...
                    else self._redact_text(str(item))
                )
    ...
    def _redact_text(self, text: str) -> str:
        ...
        # SSN
        text = re.sub(
            self.SSN_PATTERN, lambda m: self._get_redaction("ssn", m.group()), text
        )

        # Credit Cards
        text = re.sub(
            self.CREDIT_CARD_PATTERN,
            lambda m: self._get_redaction("credit_card", m.group()),
            text,
        )

        # Emails
        text = re.sub(
            self.EMAIL_PATTERN, lambda m: self._get_redaction("email", m.group()), text
        )

        # Phones
        text = re.sub(
            self.PHONE_PATTERN, lambda m: self._get_redaction("phone", m.group()), text
        )

        # Account numbers
        text = re.sub(
            self.ACCOUNT_PATTERN,
            lambda m: self._get_redaction("account", m.group()),
            text,
        )
