...
"""
    ...
    def _generate_with_template(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        detailed_analysis = self._generate_detailed_analysis(
            transactions, evidence, typology
        )
    ...
    def _generate_summary(
        self, typology: str, transactions: List[Dict], evidence: Dict
    ) -> str:
    ...
    def _format_evidence_section(self, transactions: List[Dict], evidence: Dict) -> str:
        ...
        if len(transactions) > 10:
            evidence_lines.append(
                f"  ... and {len(transactions) - 10} additional transactions"
            )
        ...
        if evidence.get("pep_hits"):
            evidence_lines.append(
                f"PEP Screening: {len(evidence['pep_hits'])} politically exposed persons identified"
            )
    ...
    def _generate_with_llm(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.llm_client.generate(
                prompt=prompt, temperature=0.0, max_tokens=2000  # Deterministic
            )
...
"""
    ...
    def _validate_and_extract_llm_citations(
