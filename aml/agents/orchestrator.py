...
class Orchestrator:
    ...
    def process_transaction_batch(self, transactions: List[Dict]) -> Dict[str, Any]:
        ...
        logger.info(
            f"[{workflow_id}] Starting batch processing for {len(transactions)} transactions"
        )
        ...
        try:
            ...
            features = self._run_agent(
                "feature_engineer", {"transactions": transactions}
            )
            ...
            suspicious_txns = self._extract_suspicious(
                transactions, classification["result"].get("predictions", [])
            )

            logger.info(
                f"[{workflow_id}] Identified {len(suspicious_txns)} suspicious transactions"
            )
            ...
            for entity_id, entity_txns in entity_groups.items():
                try:
                    ...
                    logger.error(
                        f"[{workflow_id}] Error processing entity {entity_id}: {e}"
                    )
            ...
            results["detection_rate"] = (
                len(suspicious_txns) / len(transactions) if transactions else 0
            )
    ...
    def _process_entity(
        self, entity_id: str, transactions: List[Dict], workflow_id: str
        ...
        logger.info(
            f"[{workflow_id}] Processing entity {entity_id} with {len(transactions)} transactions"
        )

        # Step 4: External Intelligence
        intelligence = self._run_agent(
            "intelligence", {"entity_id": entity_id, "transactions": transactions}
        )
        ...
        risk_score = self._calculate_risk_score(
            transactions, intelligence["result"], typology
        )
    ...
    def _extract_suspicious(
        self, transactions: List[Dict], predictions: List[int]
    ) -> List[Dict]:
    ...
    def _calculate_risk_score(
        self, transactions: List[Dict], intelligence: Dict, typology: str
    ) -> float:
        """
